...
#### There may be some restrictions in AWS lambda, such as the time the code could be running, memory, and such. These constraints may enable us to use lambda with different solutions.
...
#### We can install these packages on an EFS and connect the EFS to a Lambda function.
...
###### After doing that let's go to the EFS we created and click the 'attach' in the upper right. Copy the NFS client command paste that but without the "efs" at the end.
###### Add "mnt" instead of efs.
...
```
###### Now install the Python packages in the access folder.
 
```console      
pip3 install --upgrade --t mnt/access/ numpy
```
...
# connecting to the folder with the packages in EFS
# NOTE: imports must be after this line
sys.path.append('/mnt/access')
